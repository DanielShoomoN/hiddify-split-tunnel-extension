...
// HiddifySplitTunnelExtensionData holds the data specific to HiddifySplitTunnelExtension
type HiddifySplitTunnelExtensionData struct {
	Count int `json:"count"` // Number of counts for the extension
}

// HiddifySplitTunnelExtension represents the core functionality of the extension
type HiddifySplitTunnelExtension struct {
	ex.Base[HiddifySplitTunnelExtensionData]                    // Embedding base extension functionality
...
func (e *HiddifySplitTunnelExtension) backgroundTask(ctx context.Context) {
...
func (e *HiddifySplitTunnelExtension) SubmitData(button string, data map[string]string) error {
	switch button {
...
func (e *HiddifySplitTunnelExtension) stop() error {
	if e.cancel != nil {
...
func (e *HiddifySplitTunnelExtension) Close() error {
	return e.stop() // Simply delegate to stop
}

// To Modify user's config before connecting, you can use this function
func (e *HiddifySplitTunnelExtension) BeforeAppConnect(hiddifySettings *config.HiddifyOptions, singconfig *option.Options) error {
	return nil
}

// NewHiddifySplitTunnelExtension initializes a new instance of HiddifySplitTunnelExtension with default values
func NewHiddifySplitTunnelExtension() ex.Extension {
	return &HiddifySplitTunnelExtension{
		Base: ex.Base[HiddifySplitTunnelExtensionData]{
			Data: HiddifySplitTunnelExtensionData{ // Set default data
				Count: 4, // Default count value
			},
		},
		console: yellow.Sprint("Welcome to ") + green.Sprint("hiddify-split-tunnel-extension\n"), // Default message
	}
}
...
func init() {
	ex.RegisterExtension(
		ex.ExtensionFactory{
			Id:          "github.com/DanielShoomoN/hiddify_split_tunnel_extension/hiddify_extension", // Package identifier
			Title:       "hiddify-split-tunnel-extension",                                                         // Display title of the extension
			Description: "Awesome Extension hiddify_split_tunnel_extension created by DanielShoomoN",                                                     // Brief description of the extension
			Builder:     NewHiddifySplitTunnelExtension,                                                       // Function to create a new instance
		},
	)
}
